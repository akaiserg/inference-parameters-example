"""
LLM Inference Demo: From Logits to Next Token (CSV VERSION)
...
GENERIC VERSION: Reads tokens and logits from a CSV file!
"""
...
def load_logits_from_csv(csv_path: str) -> Tuple[Dict[str, float], str]:
    """
    Load tokens and their logits from a CSV file.
    ...
    Or with header:
        token,logit,context
        sleeping,2.8,"The cat is"
        eating,2.3,"The cat is"
    
    Args:
        csv_path: Path to the CSV file
    
    Returns:
        Tuple of (logits_dict, context_string)
            - logits_dict: Dictionary mapping token names to logit values
            - context_string: Optional context from CSV (or default message)
    ...
    with open(csv_path, 'r', encoding='utf-8') as f:
        ...
        if 'token' not in reader.fieldnames or 'logit' not in reader.fieldnames:
            raise ValueError("CSV must have 'token' and 'logit' columns")
        ...
        for row_num, row in enumerate(reader, start=2):  # Start at 2 (after header)
            ...
            if not token:
                raise ValueError(f"Row {row_num}: Empty token name")
            
            try:
                logit = float(row['logit'])
            except ValueError:
                raise ValueError(f"Row {row_num}: Invalid logit value '{row['logit']}' for token '{token}'")
            
            logits[token] = logit
            
            # Get context if available (use first row's context)
            if context is None and 'context' in row and row['context']:
    ...
    if context is None:
        context = "[Context not provided in CSV]"
...
def main(csv_path: str, temperature: float = 1.0, top_k: int = None, top_p: float = None, seed: int = None):
    """
    Main demonstration function that will orchestrate all steps.
    
    Args:
        csv_path: Path to CSV file with tokens and logits
    ...
    """
    print("=" * 70)
    print("🧠 LLM INFERENCE DEMO: From CSV to Next Token")
    print("=" * 70)
    print()
    ...
    try:
        initial_logits, context = load_logits_from_csv(csv_path)
        print(f"✅ Loaded {len(initial_logits)} tokens from: {csv_path}")
        print()
    ...
    print("-" * 70)
    print("STEP 0: Model Output (Raw Logits from CSV)")
    print("-" * 70)
    print()
    ...
    print(f'  "{context} {selected_token}"')
    print()
    print("=" * 70)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="LLM Inference Demo: Load tokens/logits from CSV and run complete pipeline",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
CSV Format:
  Required columns: token, logit
  ...
  Example CSV:
    ...
    sleeping,2.8,"The cat is"
    eating,2.3,"The cat is"
    playing,1.9,"The cat is"

Examples:
  python3 llm_inference_demo_csv.py data.csv                         # Basic usage
  python3 llm_inference_demo_csv.py data.csv -t 0.5                  # With temperature
  python3 llm_inference_demo_csv.py data.csv -k 3                    # With top-k
  python3 llm_inference_demo_csv.py data.csv -p 0.9                  # With top-p
  python3 llm_inference_demo_csv.py data.csv -t 0.7 -k 3 -p 0.9 -s 42  # Full config
        """
    )
    ...
    parser.add_argument(
        'csv_file',
        type=str,
        help='Path to CSV file with tokens and logits'
    )
    ...
    # Run the demo
    main(csv_path=args.csv_file, temperature=args.temperature, top_k=args.top_k, top_p=args.top_p, seed=args.seed)
